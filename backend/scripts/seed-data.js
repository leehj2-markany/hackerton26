...
const PRODUCTS = [
  ...
  { name: 'SafeCopy', count: 100 },
  { name: 'ContentSAFER', count: 50 },
];
...
const QUESTIONS = {
  ...
  SafeCopy: [
    'SafeCopy 출력물 보안 솔루션의 주요 기능을 알고 싶습니다.',
    '프린터별 출력 제어 정책 설정이 가능한가요?',
    '출력물 워터마크 커스터마이징 옵션에 대해 문의합니다.',
    'SafeCopy와 복합기(MFP) 연동 지원 현황을 알려주세요.',
    '출력 이력 추적 및 감사 기능에 대해 상세히 알고 싶습니다.',
    '네트워크 프린터 환경에서 SafeCopy 적용 방법을 문의합니다.',
    '출력물 승인 워크플로우 기능이 있나요?',
    'SafeCopy 에이전트 자동 배포 방법을 알려주세요.',
    '개인정보 포함 문서 출력 시 자동 차단 기능이 있나요?',
    '출력 쿼터(할당량) 관리 기능에 대해 문의합니다.',
    'SafeCopy 도입 후 출력 비용 절감 효과는 어느 정도인가요?',
    'macOS 환경에서도 SafeCopy가 지원되나요?',
  ],
  ContentSAFER: [
    'ContentSAFER의 유해 콘텐츠 필터링 정확도는 어느 정도인가요?',
    '실시간 콘텐츠 모니터링 기능에 대해 알고 싶습니다.',
    'ContentSAFER API 연동 방법과 문서를 요청합니다.',
    '이미지/동영상 콘텐츠 분석 기능이 포함되어 있나요?',
    '커스텀 필터링 규칙 설정이 가능한가요?',
    'ContentSAFER의 처리 속도(TPS)는 어느 정도인가요?',
    '다국어 콘텐츠 필터링을 지원하나요?',
    'ContentSAFER 온프레미스 설치형과 클라우드형의 차이점은?',
    '딥러닝 기반 콘텐츠 분석 모델의 업데이트 주기가 궁금합니다.',
    '대규모 트래픽 환경에서의 안정성을 보장할 수 있나요?',
  ],
};
...
const AI_ANSWERS = {
  ...
  SafeCopy: [
    'SafeCopy는 출력물 보안 솔루션으로, 출력 제어, 워터마크 삽입, 출력 이력 추적, 승인 워크플로우 등의 기능을 제공합니다.',
    ...
    '네트워크 프린터 환경에서는 프린트 서버에 SafeCopy 모듈을 설치하여 중앙 관리가 가능합니다.',
    '출력물 승인 워크플로우를 통해 상급자 승인 후 출력이 가능하도록 설정할 수 있습니다.',
    'SafeCopy 에이전트는 SCCM, GPO 등을 통한 자동 배포를 지원합니다.',
    '개인정보(주민번호, 전화번호 등) 포함 문서 출력 시 자동 감지 및 차단/경고 기능을 제공합니다.',
    '사용자별/부서별 월간 출력 쿼터를 설정할 수 있으며, 초과 시 자동 차단 또는 경고를 발생시킵니다.',
    'SafeCopy 도입 후 평균 출력량 30% 감소, 연간 출력 비용 약 25% 절감 효과를 기대할 수 있습니다.',
    'macOS 환경은 현재 베타 지원 중이며, 정식 지원은 다음 버전(v2.5)에서 제공될 예정입니다.',
  ],
  ContentSAFER: [
    'ContentSAFER의 유해 콘텐츠 필터링 정확도는 텍스트 기준 98.5%, 이미지 기준 95.2%입니다.',
    '실시간 콘텐츠 모니터링은 스트리밍 방식으로 동작하며, 평균 응답 시간 50ms 이내를 보장합니다.',
    'REST API를 통한 연동이 가능하며, API 문서와 SDK(Python, Java, Node.js)를 제공합니다.',
    '이미지 및 동영상 콘텐츠 분석 기능이 포함되어 있으며, 딥러닝 기반 객체 인식 기술을 활용합니다.',
    '커스텀 필터링 규칙은 관리자 콘솔에서 설정 가능하며, 정규식, 키워드, 카테고리 기반 규칙을 지원합니다.',
    'ContentSAFER의 처리 속도는 단일 서버 기준 약 1,000 TPS이며, 수평 확장을 통해 처리량을 늘릴 수 있습니다.',
    '한국어, 영어, 일본어, 중국어 등 주요 언어의 콘텐츠 필터링을 지원합니다.',
    '온프레미스 설치형은 데이터가 외부로 나가지 않아 보안이 강화되며, 클라우드형은 빠른 도입과 자동 업데이트가 장점입니다.',
    '딥러닝 모델은 월 1회 정기 업데이트되며, 긴급 이슈 발생 시 수시 업데이트도 진행됩니다.',
    '로드밸런서와 오토스케일링을 통해 대규모 트래픽 환경에서도 안정적인 서비스를 보장합니다.',
  ],
};
